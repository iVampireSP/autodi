...
const maxPDMethodsShown = 8

// generatePkgDiagram scans every package under the module and produces a Mermaid
// classDiagram showing exported types (structs, interfaces), their fields/methods,
...
func (g *pkgDiagramGen) render(rels []pdRelation) []byte {
	var buf bytes.Buffer

	buf.WriteString("%% Generated by autodi — DO NOT EDIT\n")
	buf.WriteString("%% Requires Mermaid v10.2+ for namespace support\n")
	buf.WriteString("classDiagram\n\n")

	for _, pkg := range g.pkgInfos {
		nsID := sanitizeMermaidID(pkg.RelPath)
		fmt.Fprintf(&buf, "    namespace %s {\n", nsID)

		// Interfaces
		for _, iface := range pkg.Ifaces {
			id := g.typeNodeID(pkg, iface.Name)
			fmt.Fprintf(&buf, "        class %s {\n", id)
			buf.WriteString("            <<interface>>\n")
			g.writeMethods(&buf, id, iface.Methods)
			buf.WriteString("        }\n")
		}

		// Structs
		for _, st := range pkg.Structs {
			id := g.typeNodeID(pkg, st.Name)
			fmt.Fprintf(&buf, "        class %s {\n", id)
			if g.isDecorator(pkg, st) {
				buf.WriteString("            <<decorator>>\n")
			}
			for _, f := range st.Fields {
				fmt.Fprintf(&buf, "            +%s %s\n", f.TypeStr, f.Name)
			}
			g.writeMethods(&buf, id, st.Methods)
			buf.WriteString("        }\n")
		}
	...
	if len(rels) > 0 {
		...
		for _, r := range rels {
			switch r.Kind {
			case "implements":
				fmt.Fprintf(&buf, "    %s ..|> %s : implements\n", r.FromID, r.ToID)
			case "depends":
				fmt.Fprintf(&buf, "    %s ..> %s : depends\n", r.FromID, r.ToID)
			case "field":
				fmt.Fprintf(&buf, "    %s --> %s\n", r.FromID, r.ToID)
			}
		}
	...
	// ── Usage counts ──────────────────────────────────────────────────────────
	// refByNode[id]  = number of types that depend on or have a field of type id
	// implByNode[id] = number of types that implement interface id
	...
	// Emit "note for" directives for any type with non-zero counts.
	// Notes must appear at the top-level (outside namespace blocks).
	hasNotes := false
	for _, pkg := range g.pkgInfos {
		for _, iface := range pkg.Ifaces {
			id := g.typeNodeID(pkg, iface.Name)
			...
			if impl > 0 || use > 0 {
				...
				fmt.Fprintf(&buf, "    note for %s \"%d impl · %d use\"\n", id, impl, use)
			}
		}
		for _, st := range pkg.Structs {
			id := g.typeNodeID(pkg, st.Name)
			...
			if use > 0 {
				...
				fmt.Fprintf(&buf, "    note for %s \"used by %d\"\n", id, use)
			}
		}
	}
...
func (g *pkgDiagramGen) writeMethods(buf *bytes.Buffer, nodeID string, methods []pdMethod) {
	shown := methods
	if len(shown) > maxPDMethodsShown {
		shown = shown[:maxPDMethodsShown]
	}
	for _, m := range shown {
		retStr := ""
		if len(m.Returns) > 0 {
		...
		fmt.Fprintf(buf, "            +%s(%s)%s\n",
			m.Name, strings.Join(m.Params, ", "), retStr)
	}
	if len(methods) > maxPDMethodsShown {
		fmt.Fprintf(buf, "    %% %s: %d methods total, showing %d\n",
			nodeID, len(methods), maxPDMethodsShown)
	}
}
